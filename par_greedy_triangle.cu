...
__global__ void find_new_small(int *smallest_idx, int *num_lines, int *indicator) {
	// Increment the index of the smallest line past all empty lines
	int idx = *smallest_idx;
	idx++;
	while (idx < *num_lines && !indicator[idx]) {
...
int main(int argc, char *argv[]) {
	...
	while (smallest_idx < num_lines) {
		...
		// Find the new smallest line on the GPU
		find_new_small<<<1,1>>>(d_smallest_idx, d_num_lines, d_indicator);
		cudaMemcpy(&smallest_idx, d_smallest_idx, sizeof(int), cudaMemcpyDeviceToHost);
	}
	// Get the indicator array so we know which lines belong to the triangulation.
	...
	// Only display the time for the triangulation step.
	printf("%.4f", GET_TIMER(triangulate));
