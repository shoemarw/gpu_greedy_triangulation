...
__device__ bool d_conflicts(double *I, double *J) {
	// if (threadIdx.x == 6) {
	// 	printf("Thread %d inside d_conflicts.\n(%lf, %lf) (%lf,%lf)\n(%lf, %lf) (%lf,%lf)\n", threadIdx.x,
	// 		I[0], I[1], I[2], I[3], 
	// 		J[0], J[1], J[2], J[3]);
	// 	printf("Do they share endpoints? %d\n", d_share_endpoint(I,J));
	// 	printf("Do they intersect? %d\n\n", d_intersects(I,J));
	// }
	return !d_share_endpoint(I, J) && d_intersects(I, J);
}

__global__ void triangulate(double *L, int *num_l) {
	int k = threadIdx.x; // A thread's index
	int num_treads = 1024; // Number of threads
	...
	// __syncthreads();
	// if (threadIdx.x == 0){ 
	// 	printf("Lines on device before Triangulation:\n");
	// 	d_print_lines(L, *num_l);
	// }
	// __syncthreads();
	...
	while (smallest_idx < *num_l) {
		for (int j = lb_k; j < ub_k; j++) {
			...
			if (j < *num_l) {
				...
				//if (threadIdx.x == 6) {
					// printf("Thread %d checking line (%lf, %lf) (%lf, %lf)\nfor conflicts with (%lf, %lf) (%lf, %lf)\n\n", 
					// 	k, L[4*j], L[4*j+1], L[4*j+2], L[4*j+3], L[4*smallest_idx], L[4*smallest_idx+1],
					// 	L[4*smallest_idx+2], L[4*smallest_idx+3]);
				//}
				if (d_conflicts(&L[4*smallest_idx], &L[4*j])) {
					// There is an intersection so set line j to be
					// the 'empty line'
					// printf("Thread %d eliminated line (%lf, %lf) (%lf, %lf)\nfor conflicts with (%lf, %lf) (%lf, %lf)\n\n", 
					// k, L[4*j], L[4*j+1], L[4*j+2], L[4*j+3], L[4*smallest_idx], L[4*smallest_idx+1],
					// L[4*smallest_idx+2], L[4*smallest_idx+3]);
					L[4*j]     = 0;
					L[4*j + 1] = 0;
	...
	// __syncthreads();
	// if (threadIdx.x == 0){ 
	// 	printf("Lines on device after Triangulation:\n");
	// 	d_print_lines(L, *num_l);
	// 	printf("num_l = %d\n", *num_l);
	// }
	// __syncthreads();
...
int main(int argc, char *argv[]) {
	...
	// printf("Lines stored in segments: \n");
	// print_lines(segments, num_lines);
