...
__global__ void triangulate(double *L, double *T, double *temp, int *num_l, int *num_th) {
	int k = threadIdx.x; // A thread's index
	int num_treads = *num_th; // Number of threads
	...
	while (smallest_idx < *num_l) {
		if (k >= *num_l) {
			// Have all unnecessary treads to break out of the
			// while loop.
			break;
		}
		for (int j = lb_k; j < ub_k; j++) {
			...
			if (j >= *num_l) {
				break; // Reduce unnecessary iterations
			}
			// Run intersection test
			if (d_conflicts(&L[4*smallest_idx], &L[4*j])) {
				// There is an intersection between line j and the smallest
				// line, so set line j to be the 'empty line'
				L[4*j]     = 0;
				L[4*j + 1] = 0;
				L[4*j + 2] = 0;
				L[4*j + 3] = 0;
		...
		__syncthreads();
		if (threadIdx.x == 0) {
			// Store the smallest line in the triangulation.
			T[4*triang_count]   = L[4*smallest_idx];
			T[4*triang_count+1] = L[4*smallest_idx+1];
			T[4*triang_count+2] = L[4*smallest_idx+2];
			T[4*triang_count+3] = L[4*smallest_idx+3];
			triang_count++; 
			// Update the index of the smallest line to refer
			// to the next smallest line that is non-empty.
			smallest_idx++;
			while(smallest_idx < *num_l
			   && L[4*smallest_idx]     == 0 
		...
		// Sync all threads
		__syncthreads();
		// Copy all lines past and including the smallest line
		// that are non-empty (so we forget the previous smallest
		// line and we forget the empty lines.)
		int line_count = 0;
		// Perform the copy in parallel
		for (int i = lb_k + smallest_idx; i < ub_k; i++) {
			if (i < *num_l) {
				if (   !(L[4*i] == 0)   || !(L[4*i+1] == 0) 
					|| !(L[4*i+2] == 0) || !(L[4*i+3] == 0)) {
					temp[4*line_count]   = L[4*i];
					temp[4*line_count+1] = L[4*i+1];
					temp[4*line_count+2] = L[4*i+2];
					temp[4*line_count+3] = L[4*i+3];
					line_count++;//////////////////////////////////protect this variable
				}
			}
		}
		// Copy over all non-empty lines back to L
		memcpy(L, temp, line_count*4*size);
		// Update the number of lines
		*num_l = line_count;
		// Reset the index of the smallest line to be the 
		// first line in L
		smallest_idx = 0;
	}
	if (threadIdx.x == 0) {
		// Update num_l so that the host knows how many lines are
...
int main(int argc, char *argv[]) {
	...
	triangulate<<<1, num_threads>>>(d_lines, d_triang, d_num_lines, d_num_threads);
	cudaDeviceSynchronize();

	// Read in the triangulation from the device
	cudaMemcpy(&num_lines, d_num_lines, sizeof(int), cudaMemcpyDeviceToHost);
	double* triang   = (double*) allocate(num_lines*size*4);
	cudaMemcpy(triang, d_triang, num_lines*4*size, cudaMemcpyDeviceToHost);
